...
namespace CartoonFX
{
    namespace CustomShaderImporter
    {
        ...
        public class CFXR_ShaderImporter : ScriptedImporter
        {
            ...
            {
                var startsWithTag = line.Length > 4 && line[0] == '/' && line[1] == '*' && line[2] == '*' &&
                                    line[3] == '*';
                if (startsWithTag) return true;

                var l = line.Length - 1;
                var endsWithTag = line.Length > 4 && line[l] == '/' && line[l - 1] == '*' && line[l - 2] == '*' &&
                                  line[l - 3] == '*';
                return endsWithTag;
            }
                ...
                public override void OnInspectorGUI()
                {
                        ...
                        && ((isUsingURP && !compiledForURP) || (!isUsingURP && compiledForURP)))
                    {
                        ...
                        EditorGUILayout.HelpBox(
                            ...
                            MessageType.Warning);
                        if (GUILayout.Button("Reimport Shader")) shouldReimportShader = true;
                        GUI.color = guiColor;
                    }
                    ...
                    if (EditorGUI.EndChangeCheck()) shouldReimportShader = true;

                    if (GUILayout.Button("View Source", GUILayout.ExpandWidth(false)))
                    {
                        var path = Application.temporaryCachePath + "/" + Importer.shaderName.Replace("/", "-") +
                                   "_Source.shader";
 ...
 EditorUtility.SaveFilePanel("Export CFXR shader", Application.dataPath, "CFXR Shader","shader");
                    ...
                    if (shouldReimportShader) ReimportShader();
                }
